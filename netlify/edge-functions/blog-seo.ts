...
function generateSEOTags(post: BlogPost): string {
  ...
  const description = post.meta_description || post.excerpt || `Lees "${post.title}" op Algarve voor Bijna Niks - expert reisadvies voor de Algarve.`;
  const canonicalUrl = `https://lokaalalgarve.nl/blog/${post.slug}`;
  ...
  return `
    ...
    <script type="application/ld+json">
    {
      ...
      "@type": "BlogPosting",
      "headline": "${escapeHtml(title.replace(' - Algarve voor Bijna Niks', ''))}",
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://lokaalalgarve.nl/og-image.jpg"
        }
      },
      "mainEntityOfPage": {
        "@type": "WebPage",
